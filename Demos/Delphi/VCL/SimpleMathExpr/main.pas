...
type
  ...
  private
    ...
    FJsonVars: TQJson;
    procedure DoGetYValue(Sender: TObject; AVar: TQMathVar;
      const ACallParams: PQEvalData; var AResult: Variant);
    procedure DoStableValue(Sender: TObject; AVar: TQMathVar;
      const ACallParams: PQEvalData; var AResult: Variant);
    procedure DoGetJsonVar(Sender: IQMathExpression; const AVarName: String;
      var AVar: TQMathVar);
    class procedure DoGetJsonValue(Sender: TObject; AVar: TQMathVar;
  ...
  private
    procedure DoGetJsonVar(Sender: IQMathExpression; const AVarName: String;
      var AVar: TQMathVar);
    procedure DoGetJsonValue(Sender: TObject; AVar: TQMathVar;
      const ACallParams: PQEvalData; var AResult: Variant);
...
procedure TForm1.DoGetYValue(Sender: TObject; AVar: TQMathVar;
  const ACallParams: PQEvalData; var AResult: Variant);
var
  S: String;
begin
  if InputQuery('输入 Y 的值', '计算表达式需要知道Y值：', S) then
    AResult := StrToFloat(S);
end;

procedure TForm1.DoStableValue(Sender: TObject; AVar: TQMathVar;
  const ACallParams: PQEvalData; var AResult: Variant);
var
  V: Integer;
begin
...
procedure TQJsonVarHelper.DoGetJsonValue(Sender: TObject; AVar: TQMathVar;
  const ACallParams: PQEvalData; var AResult: Variant);
var
  AItem: TQJson;
begin
...
begin
  ...
  if StartWithW(PWideChar(AVarName), '$', false) then
    AItem := AJson.ItemByPath(RightStrW(AVarName, Length(AVarName) - 1, false))
  else
