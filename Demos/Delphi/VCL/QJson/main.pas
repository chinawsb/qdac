...
begin
  ...
  try
    ...
    mmResult.Lines.Add('默认输出 :' + AJson.CatValues('"', ',', []));
    mmResult.Lines.Add('忽略重复 :' + AJson.CatValues('"', ',',
      [coIgnoreDuplicates]));
    mmResult.Lines.Add('忽略空白 :' + AJson.CatValues('"', ',', [coIgnoreEmpty]));
    mmResult.Lines.Add('忽略重复+空白 :' + AJson.CatValues('"', ',',
      [coIgnoreDuplicates, coIgnoreEmpty]));
    mmResult.Lines.Add
      ('更多用法，参考 TQJson 的 CatValues,CateNames,FlatNames,FlatValues,NameToStrings,ValueToStrings 声明');
...
begin
  AJson:=AcquireJson;
  try
    AJson.ValuesFromStrings(['Hello,world','Helow,earth','Hello,me'],true,0,MaxInt);//后面三个参数有默认值
    mmResult.Text:=AJson.AsJson;
