...
procedure TQDataSet.ClearCalcFields(Buffer: PByte);
var
  ARec: TQRecord;
  I: Integer;
begin
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ADef := FieldDefs[I] as TQFieldDef;
        AField := ASource.Fields[ACopyFields[I].Index];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AValue := @ARec.Values[ACopyFields[J].Index];
  ...
  try
    ...
    begin
      ...
      ADest.Values[FieldDefs.Find(ASrc.Values[AColIdx].CurrentValue^.AsString)
        .Index].OldValue.Copy(ASrc.Values[AValIdx].CurrentValue^, False);
    end;
  finally
...
var
  ...
  begin
    ...
    begin
      AColVal := @ASource.Values[AFields[J].Index];
...
begin
  ...
  begin
    ...
    if Assigned(ADef) then
      Inc(ASize, ARec.Values[ADef.Index].CurrentValue.Size);
  end;
  ...
  begin
    ADef := AFields[I];
    if Assigned(ADef) then
      WriteValue(ARec.Values[ADef.Index].CurrentValue);
  end;
end;
{$IFDEF UNICODE}
...
var
  ASource: PQValue;
begin
  if ARecord.Values[AField.Index].Changed then
    ASource := @ARecord.Values[AField.Index].NewValue
  else
    ASource := @ARecord.Values[AField.Index].OldValue;
  ...
  case Result of
    grOK:
      begin
        ...
        GetCalcFields(TRecordBuffer(AOrigin));
      end;
    grError:
      begin
    ...
    begin
      ...
      begin
        ...
        if Assigned(ASourceFields[J]) then
          ASVal := ASource.Values[ASourceFields[J].Index].CurrentValue
        else
          ASVal := nil;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Length(ARecord.FValues) <= AField.Index then
          Exit;
        if AIsOld or (not ARecord.Values[AField.Index].Changed) then
          AValue := @ARecord.Values[AField.Index].OldValue
        else
          AValue := @ARecord.Values[AField.Index].NewValue;
...
begin
  if (AField.Index >= 0) and Assigned(ARecord) then
  begin
...
var
  ...
  begin
    ...
    for J := 0 to FieldDefs.Count - 1 do
    begin
      AColVal := @ASource.Values[AFields[J].Index - 1];
      if AColVal.Changed then
        R2.Values[J].NewValue.Copy(AColVal.NewValue, False);
      if R2.Status <> usInserted then
        R2.Values[J].OldValue.Copy(AColVal.OldValue, False);
    ...
    begin
      ...
      begin
        if loPartialKey in Options then
          Result := StrIStrW(PQCharW(ARec.Values[AFields[I].Index]
            .CurrentValue^.AsString), PQCharW(AValues[I])) <> nil
        else
          Result := StrCmpW(PQCharW(ARec.Values[AFields[I].Index]
            .CurrentValue^.AsString), PQCharW(AValues[I]), True) = 0;
      end
      else
      begin
        if loPartialKey in Options then
          Result := StrStrW(PQCharW(ARec.Values[AFields[I].Index]
            .CurrentValue^.AsString), PQCharW(AValues[I])) <> nil
        else
          Result := StrCmpW(PQCharW(ARec.Values[AFields[I].Index]
...
begin
  ...
  begin
    ...
    AVal.Copy(ARec.Values[ADef.Index].CurrentValue^, False);
    for I := 1 to FActiveRecords.Count - 1 do
    begin
      ARec := FActiveRecords[I];
      if ADef.ValueComparor(@AVal, ARec.Values[ADef.Index].CurrentValue) < 0
      then
        AVal.Copy(ARec.Values[ADef.Index].CurrentValue^, False);
    end;
      ...
      usUnmodified:
        begin
          ...
          begin
            ...
            begin
              ASrcIdx := AFields[J].Index;
              if not ASrc.Values[ASrcIdx].OldValue.IsNull then
              begin
                ADestIdx := FieldDefs[J].Index;
...
procedure TQDataSet.SetFieldValue(ARecord: PQRecord; AField: TField;
  const AValue: TQValue);
begin
  ARecord.Values[AField.Index].NewValue.Copy(AValue, True);
end;
...
begin
  ...
  while Assigned(AExp) do
  begin
    AVal1 := ARec1.Values[AExp.Field.Index].CurrentValue;
    AVal2 := ARec2.Values[AExp.Field.Index].CurrentValue;
...
function TQFilterExp.Accept(ARecord: TQRecord;
  ...
  begin
    ...
    begin
      ...
      begin
        ACol := ARecord.Values[AExp.Field.Index];
...
begin
  ...
  begin
    ...
    FOriginValue := ABuf.Values[FField.Index].CurrentValue;
  ...
  begin
    ...
    begin
      try
        ...
        if Assigned(ABuf.Bookmark) then
          ABuf := ABuf.Bookmark;
        AValues := @ABuf.Values[FField.Index];
