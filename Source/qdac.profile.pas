...
uses Classes, Sysutils, TimeSpan, Diagnostics, SyncObjs, Generics.Defaults, Generics.Collections,
  qdac.profile.base;
...
type
  ...
  PQProfileCallback = ^TQProfileCallback;
  ...
  // 回调链表节点
  TQProfileCallback = record
    Callback: TQProfileTimeEscapeCallback;
    Next: PQProfileCallback;
  end;
  ...
  public
    ...
    // TQProfileBase — 文本报告（同 AsString）
    class function Report: string; override;
    // TQProfileBase — JSON 报告（同 AsString）
    ...
    class function Calc(
        const AName: string;
        AStackRef: PQProfileStack = nil;
        ACallback: TQProfileTimeEscapeCallback = nil
...
begin
  ...
  if AStacks.MaxLevel > 0 then begin
...
begin
  ...
  // 返回实际记录的帧数（已跳过需忽略的帧数）
  if AItems.Count > AItems.Skip then
    Result := AItems.Count - AItems.Skip
  else
    Result := 0;
end;
{$ENDIF}
...
class function TQProfile.Calc(
    const AName: string;
    AStackRef: PQProfileStack;
    ACallback: TQProfileTimeEscapeCallback
): IQProfileHelper;
var
  PCallback: PQProfileCallback;
begin
  if Enabled then begin
    Result := TQThreadHelperSet.NeedHelper(TThread.Current.ThreadId, AName, AStackRef, nil);
    if Assigned(ACallback) then begin
      New(PCallback);
      PCallback.Callback := ACallback;
      PCallback.Next := PQProfileStack(Result.CurrentStack).Callbacks;
      PQProfileStack(Result.CurrentStack).Callbacks := PCallback;
    end;
  end
  else
    Result := nil;
...
class procedure TQProfile.Cleanup;
  ...
  var
    ...
    ACallback, ANextCallback: PQProfileCallback;
  begin
    AChild := AStack.FirstChild;
    while Assigned(AChild) do begin
      ANext := AChild.Next;
      // 清空回调
      ACallback := AChild.Callbacks;
      while Assigned(ACallback) do begin
        ANextCallback := ACallback.Next;
        Dispose(ACallback);
        ACallback := ANextCallback;
      end;
      // 清空引用
      ARef := AChild.FirstRef;
      while Assigned(ARef) do begin
        ANextRef := ARef.Next;
        Dispose(ARef);
        ARef := ANextRef;
      end;
...
var
  I: Integer;
begin
  for I := 0 to High(TQThreadHelperSet.FHelpers) do begin
    if Assigned(TQThreadHelperSet.FHelpers[I]) then begin
...
class procedure TQProfile.Reset;
begin
  Cleanup;
end;
...
{$IFDEF DEBUG}
      true
{$ELSE}
      FindCmdlineSwitch('EnableProfile')
{$ENDIF}
      ;
...
begin
  case TimeUnit of
    tuDefault: Result := IntToStr(ADuration);
    tuMilliSecond: Result := FormatFloat('0.######', EscapedTimeMs(ADuration))
  end;
end;
...
var
  ...
  begin
    ...
    while Assigned(AItem) do begin
      ...
      for I := 0 to High(ANameArray) do begin
        ATemp.Name := ANameArray[I];
        if not TArray.BinarySearch<TQProfileFunctionStatics>(Result.Functions, ATemp, J, AComparer) then begin
          ...
          with Result.Functions[J] do begin
...
begin
  ...
  ACount := 0;
  AComparer :=
      TComparer<TQProfileFunctionStatics>.Construct(
          function(const L, R: TQProfileFunctionStatics): Integer begin Result := CompareStr(L.Name, R.Name); end
      );
  for I := 0 to High(TQThreadHelperSet.FHelpers) do begin
    if Assigned(TQThreadHelperSet.FHelpers[I]) then begin
...
class function TQProfile.GetEnabled: Boolean;
begin
  Result := FEnabled;
end;

class function TQProfile.GetFileName: string;
begin
  Result := FFileName;
end;
...
begin
  if (Length(AStack.Name) = 0) and Assigned(AStack.InvokeSource) then begin
...
var
  ...
  begin
    ATargetIdx := 0;
    if Assigned(AStack.Parent) then begin
      // 我们以JSON格式来保存
      ...
      begin
        ...
        for I := 1 to High(ANameArray) do begin
          AFunctionEntry.Name := ANameArray[I];
          if TArray
              .BinarySearch<TQProfileFunctionStatics>(AStatics.Functions, AFunctionEntry, ASourceIdx, AComparer) then
            ABuilder.Append('fn').Append(ASourceIdx).Append('-.->fn').Append(ATargetIdx).Append(SLineBreak);
        end;
      end;
      ARef := AStack.FirstRef;
      while Assigned(ARef) do begin
        if Length(ANameArray) > 1 then begin
          AFound := false;
          for I := 1 to High(ANameArray) do begin
            if ANameArray[I] = ARef.Ref.Name then begin
              AFound := true;
              break;
            end;
          end;
...
begin
  ...
  try
    ...
    AComparer :=
        TComparer<TQProfileFunctionStatics>.Construct(
            function(const L, R: TQProfileFunctionStatics): Integer begin Result := CompareText(L.Name, R.Name); end
        );
    ABuilder.Append('flowchart TB').Append(SLineBreak);
    ABuilder.Append('start(("').Append(Translation.Start).Append('"))').Append(SLineBreak);
    // 插入线程结点
    for I := 0 to High(AStatics.Threads) do begin
      ABuilder
          .Append('thread')
          .Append(AStatics.Threads[I].ThreadId)
          .Append('[["`')
          .Append(Translation.Thread)
          .Append(' ')
          .Append(AStatics.Threads[I].ThreadId)
          .Append(SLineBreak)
          .Append(EscapedText(TickDiff(FStartupTime, AStatics.Threads[I].FirstTime)))
          .Append('->')
          .Append(EscapedText(TickDiff(FStartupTime, AStatics.Threads[I].LatestTime)))
          .Append('`"]]')
          .Append(SLineBreak);
      ABuilder.Append('start-->thread').Append(AStatics.Threads[I].ThreadId).Append(SLineBreak);
    end;
    // 插入函数名结点
    for I := 0 to High(AStatics.Functions) do begin
      ABuilder
          .Append('fn')
          .Append(I)
          .Append('(')
          .Append(AnsiQuotedStr(AStatics.Functions[I].Name, '"'))
          .Append(')')
          .Append(SLineBreak);
    end;
    for I := 0 to High(TQThreadHelperSet.FHelpers) do begin
  ...
  begin
    ...
    if Assigned(AStack.Parent) then begin
      ...
      ABuilder
          .Append(ANextIndent)
          .Append('"minTime":')
          .Append(EscapedText(AStack.MinTime))
          .Append(',')
          .Append(SLineBreak);
      ABuilder
          .Append(ANextIndent)
          .Append('"maxTime":')
          .Append(EscapedText(AStack.MaxTime))
          .Append(',')
          .Append(SLineBreak);
      ABuilder
          .Append(ANextIndent)
          .Append('"totalTime":')
          .Append(EscapedText(AStack.TotalTime))
          .Append(',')
          .Append(SLineBreak);
      ABuilder
          .Append(ANextIndent)
      ...
      ABuilder
          .Append(ANextIndent)
          .Append('"firstTime":')
          .Append(EscapedText(TickDiff(FStartupTime, AStack.FirstStartTime)))
          .Append(',')
          .Append(SLineBreak);
      ABuilder
          .Append(ANextIndent)
      ...
      if Assigned(AStack.FirstChild) then begin
      ...
      // 关联引用
      if Assigned(AStack.FirstRef) or (Length(ANameArray) > 1) then begin
        ABuilder.Append(',').Append(SLineBreak);
      ...
      ABuilder.Append(AIndent).Append('{').Append(SLineBreak);
      with AStack.ThreadHelper do begin
        ABuilder.Append(ANextIndent).Append('"threadId":').Append(FThreadId).Append(',').Append(SLineBreak);
        ABuilder
            .Append(ANextIndent)
            .Append('"startTime":')
            .Append(EscapedText(TickDiff(FStartupTime, FirstTime)))
            .Append(',')
            .Append(SLineBreak);
        ABuilder
            .Append(ANextIndent)
            .Append('"latestTime":')
            .Append(EscapedText(TickDiff(FStartupTime, LatestTime)))
            .Append(',')
            .Append(SLineBreak);
      end;
...
begin
  ...
  try
    ...
    case TimeUnit of
      tuDefault: ABuilder.Append('"timeUnit":"default",').Append(SLineBreak);
      tuMilliSecond: ABuilder.Append('"timeUnit":"ms",').Append(SLineBreak);
    end;
...
class function TQProfile.Report: string;
begin
  Result := AsString;
end;

class function TQProfile.ReportJson: string;
begin
  Result := AsString;
end;
...
begin
  ...
  if Result > 0 then begin
    ...
    if FCurrent.AddedRefCount = 0 then begin
      Dec(FCurrent.NestLevel);
      if FCurrent.NestLevel = 0 then begin
...
begin
  ...
  Inc(Runs);
  while Assigned(Callbacks) do begin
    ACallback := Callbacks;
    ACallback.Callback(EscapedTimeMs(ADelta));
    Callbacks := ACallback.Next;
    Dispose(ACallback);
  end;
end;

function TQProfile.TQProfileStackHelper.Push(
    const AName: string;
    AStackRef: PQProfileStack;
    const Addr: Pointer
): PQProfileStack;
var
  ...
  begin
    if Assigned(Addr) and (Addr = InvokeSource) then
      // 地址匹配：如果也有名称，再验证名称一致
      Result := (Length(AName) = 0) or (CompareStr(Name, AName) = 0)
...
begin
  if IsNest then begin
    ...
    AChild := FirstChild;
    while Assigned(AChild) do begin
      if AChild.InvokeSource = Addr then begin
...
class function TQProfile.TQThreadHelperSet.NeedHelper(
    const AThreadId: TThreadId;
    const AName: string;
    AStackRef: PQProfileStack;
    const Addr: Pointer
...
const
  ...
  begin
    if Length(AHelpers) = 0 then
      Exit(BUCKET_MASK);
    AHash := (Integer(AThreadId) xor Integer(UInt64(AThreadId) shr 32)) and $7FFFFFFF;
    AHash := AHash mod Length(AHelpers);
    I := AHash;
    while I < Length(AHelpers) do begin
      AItem := AHelpers[I];
  ...
  **修复建议**：在`ReallocArray`过程中使用写锁（`FLocker.BeginWrite`）确保独占访问，防止并发修改。
  ...
  begin
    case Length(FHelpers) of
      0: SetLength(ANew, 19);
      19: SetLength(ANew, 67);
      67: SetLength(ANew, 131);
      131: SetLength(ANew, 509);
      509: SetLength(ANew, 1021);
      1021: SetLength(ANew, 2039)
    else
  ...
  function FindExists(out AHelper: TQThreadProfileHelper): Boolean;
  var
  ...
  begin
    ...
    if ABucketIndex >= 0 then
      AHelper := FHelpers[ABucketIndex];
    Result := Assigned(AHelper);
    FLocker.EndRead;
  end;
...
begin
  if not FindExists(AHelper) then
