...
var
  ...
  function AddRttiField(var AResult: TQSerializeField; AField: TRttiDataMember): Boolean;
  var
    I: Integer;
  ...
  begin
    ...
    if not Assigned(AFieldType) then
      Exit;
    if (AField.IsWritable) and (not (AFieldType.TypeKind in [tkUnknown, tkMethod, tkPointer, tkProcedure])) then begin
      ...
      case AFieldType.TypeKind of
        ...
        tkArray: begin
          if not Assigned(AResult.TypeData.BaseTypeData.elType) then
            AResult.TypeData.ElementType := AResult.TypeData.BaseTypeData.elType^
          else
            AResult.TypeData.ElementType := AResult.TypeData.BaseTypeData.elType2^;
...
var
  ...
  AIdx: NativeInt;
begin
  Result := TArray.BinarySearch<TQSerializeField>(FCurrent.Fields.Fields, AField, AIdx);
  if Result then
