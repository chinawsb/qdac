...
{ FMX VirtualTree 是一个基于 FMX 设计优化的一种树形结构显示控件，它可以被用来当做
  普通的表格或者是树形结构使用。
  //Todo:设置RootNodeCount=0时，FocusChanging计数有问题
}

interface

uses System.Classes, System.Sysutils, System.Types, System.UITypes,
  System.Math.Vectors, // QString,
...
type
  ...
  IQVTInplaceEditor = interface
    ...
    property Node: TQVTNode read GetNode;
...
{$M+}
  ...
  TQVTBaseEditor = class(TComponent, IInterface, IQVTInplaceEditor)
  private
  ...
  IQVTTextEditor = interface
    ['{E6B32C6F-A5CE-4347-9272-D6D412795C13}']
    function GetText: String;
  ...
  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TQVTTextEditor = class(TQVTBaseEditor, IQVTTextEditor)
  protected
  ...
  IQVTListEditor = interface(IQVTTextEditor)
    ['{0B436DD2-B63F-4A4D-AE39-A09DABA693D2}']
    ...
    property ItemObject: TObject read GetItemObject;
  end;

  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TQVTListEditor = class(TQVTBaseEditor, IQVTListEditor)
  private
  ...
  TQVTBooleanEditor = class(TQVTBaseEditor)

  end;

  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TQVTImageEditor = class(TQVTBaseEditor)
    // 实现图片编辑框
  end;
  ...
  TQVTNodeDeleteEvent = procedure(Sender: TQVirtualTreeView; ANode: TQVTNode;
    var Accept: Boolean) of object;
  TQVTColumnDeleteEvent = procedure(Sender: TQVirtualTreeView;
    AColumn: TQVTColumn; var Accept: Boolean) of object;
  ...
  TQVTState = (tsScrolling, tsDragging, tsRowSizing, tsColSizing,
    tsVisibleChanged, tsFocusChanging, tsContentChanged);
  ...
  TQVTGetCellHintEvent = procedure(Sender: TQVirtualTreeView; ANode: TQVTNode;
    AColumn: Integer; var ANodeHint: String) of object;
  TQVTNodeList = TList<TQVTNode>;
  TQTreeEditAction = (eaAppendRow, eaInsertRow, eaAppendCol, eaInsertCol,
    eaDeleteRow, eaDeleteCol);
  TQTreeEditActions = set of TQTreeEditAction;

  [ComponentPlatformsAttribute(AllCurrentPlatforms)]
  TQVirtualTreeView = class(TPresentedTextControl, IQVTDrawable)
  private
  class var
    DefaultDrawers: array [TQVTDrawerType] of IQVTDrawer;
    DefaultEditors: array [TQVTDrawerType] of IQVTInplaceEditor;
  ...
  protected
    ...
    FFocusNode, FFocusingNode: TQVTNode;
    FHoverNode: TQVTNode;
    ...
    FHeader: TQVTHeader;
    FNodeIndent: Single;
    FFocusColumn, FFocusingColumn: Integer;
    FFocusingRefCount: Integer;
    ...
    FAllowEditActions: TQTreeEditActions;
    ...
    FBeforeDeleteNode: TQVTNodeDeleteEvent;
    FBeforeDeleteColumn: TQVTColumnDeleteEvent;
    ...
    FOnGetCellHint: TQVTGetCellHintEvent;
    function GetIsEditing: Boolean;
    ...
    procedure InitNode(ANode: TQVTNode); virtual;
    ...
    procedure NodeVisibleChanged(ARecalcVisibleNodes: Boolean = false); virtual;
    procedure NodeContentChanged(ARecalcVisibleNodes: Boolean = false); virtual;
    ...
    function CreateEditor(ANode: TQVTNode; ACol: Integer)
      : IQVTInplaceEditor; virtual;
    ...
    function ChangeFocus(ANewNode: TQVTNode; ACol: Integer): Boolean;
    procedure ColumnsChanged(ARecalcVisibleNodes: Boolean = false); virtual;
    function CanDeleteNode(ANode: TQVTNode): Boolean;
    function GetIsFocused: Boolean; reintroduce;
  published
    ...
    property AllowEditActions: TQTreeEditActions read FAllowEditActions
      write FAllowEditActions;
    // Events
    ...
    property OnGetCellHint: TQVTGetCellHintEvent read FOnGetCellHint
      write FOnGetCellHint;
    property BeforeDeleteNode: TQVTNodeDeleteEvent read FBeforeDeleteNode
      write FBeforeDeleteNode;
    property BeforeDeleteColumn: TQVTColumnDeleteEvent read FBeforeDeleteColumn
      write FBeforeDeleteColumn;
    // inherited event
...
{$ENDIF}
  public
    ...
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function GetCellData(ANode: TQVTNode; AColIndex: Integer)
      : IQVTCellData; virtual;
    function GetCellDrawer(ANode: TQVTNode; AColIndex: Integer)
      : IQVTDrawer; virtual;
    function GetChildCount(ANode: TQVTNode): Integer;
    ...
    class function GetDefaultEditor(AType: TQVTDrawerType): IQVTInplaceEditor;
    ...
    property IsFocused: Boolean read GetIsFocused;
    property InplaceEditor: IQVTInplaceEditor read FInplaceEditor;
  end;
...
type
  ...
  TQCheckBounds = class(TInterfacedObject)
  protected
    FBounds: TRectF;
  public
...
begin
  ...
  begin
    ...
    FTextSettingsInfo.TextSettings.Assign(TQVTColumnTitle(src)
      .FTextSettingsInfo.TextSettings);
  end;
end;
...
function TQVirtualTreeView.CanDeleteNode(ANode: TQVTNode): Boolean;
begin
  Result := True;
  if Assigned(FBeforeDeleteNode) then
    FBeforeDeleteNode(Self, ANode, Result);
end;
...
function TQVirtualTreeView.ChangeFocus(ANewNode: TQVTNode;
  ACol: Integer): Boolean;
begin
  Result := FocusChanging(ANewNode, ACol);
  try
    FocusNode := ANewNode;
    FocusColumn := ACol;
  finally
...
procedure TQVirtualTreeView.ColumnsChanged(ARecalcVisibleNodes: Boolean);
begin
  NodeContentChanged(ARecalcVisibleNodes);
end;
...
begin
  ...
  DefaultEditors[dtText] := TQVTTextEditor.Create(nil);
  DefaultEditors[dtCheck] := TQVTListEditor.Create(nil);
  DefaultEditors[dtRadio] := TQVTListEditor.Create(nil);
  DefaultEditors[dtImage] := TQVTImageEditor.Create(nil);
  DefaultEditors[dtStateText] := TQVTListEditor.Create(nil);
  DefaultEditors[dtTree] := DefaultEditors[dtText];
  DefaultEditors[dtProgress] := DefaultEditors[dtText];
  DefaultEditors[dtMoney] := DefaultEditors[dtText];
  DefaultEditors[dtRowIndicator] := nil;
  DefaultEditors[dtHeader] := nil;
  DefaultEditors[dtCustom] := nil;
end;
...
begin
  Dec(FFocusingRefCount);
  if FFocusingRefCount = 0 then
  begin
    FStates := FStates - [tsFocusChanging];
    FFocusingNode := nil;
    FFocusingColumn := -1;
...
begin
  if tsFocusChanging in FStates then
    Result := (ANewNode = FFocusingNode) and (ANewCol = FFocusingColumn)
  else
  begin
    Result := True;
    FStates := FStates + [tsFocusChanging];
    try
      FFocusingNode := ANewNode;
      FFocusingColumn := ANewCol;
...
begin
  ...
  if not Result then
  begin
    if ((nsHover in ANode.States) or (AColumn = FHoverColumn)) and
      (poHover in PaintOptions) then
    begin
...
class function TQVirtualTreeView.GetDefaultEditor(AType: TQVTDrawerType)
  : IQVTInplaceEditor;
begin
  Result := DefaultEditors[AType];
end;
...
function TQVirtualTreeView.GetHintString: string;
var
  AHit: TQVTHitTestResult;
  ANode: TQVTNode;
  AColumn: Integer;
  APos: TPointF;
  AService: IFMXMouseService;
begin
  Result := '';
  if Assigned(OnGetCellHint) and
    TPlatformServices.Current.SupportsPlatformService(IFMXMouseService, AService)
  then
  begin
    APos := AService.GetMousePos;
    AHit := DoHitTest(ScreenToLocal(APos), ANode, AColumn);
    if AHit in [hrNode] then
      OnGetCellHint(Self, ANode, AColumn, Result);
  end;
...
function TQVirtualTreeView.GetIsFocused: Boolean;
begin
  if Assigned(FInplaceEditor) then
    Result := FInplaceEditor.IsFocused
  else
    Result := inherited IsFocused;
end;
...
function TQVirtualTreeView.HasHint: Boolean;
begin
  if ParentShowHint then
  begin
    if ParentControl <> nil then
      Result := ParentControl.ShowHint
...
begin
  ...
  case Key of
    ...
    vkInsert: // 添加一行
      begin
        if (AllowEditActions <> []) and (not Assigned(FInplaceEditor)) then
        begin
          // Insert a new row after current focused node
          if Assigned(FocusNode) then
          begin
            if Assigned(FocusNode.Next) then
            begin
              if eaInsertRow in AllowEditActions then
                FocusNode := FocusNode.Insert(ipAfter);
              Exit;
            end
          end;
          if [eaInsertRow, eaAppendRow] * AllowEditActions <> [] then
          begin
            RootNodeCount := RootNodeCount + 1;
            FocusNode := RootNode.GetLastChild;
          end;
        end;
      end;
    vkDelete:
      begin
        if (eaDeleteRow in AllowEditActions) and (not Assigned(FInplaceEditor))
          and Assigned(FocusNode) then
        begin
          FocusNode.Delete;
        end;
      end
          ...
          try
            ...
            begin
              ...
              begin
                ...
                begin
                  Selections.Clear;
                  if not(TQVTNodeState.nsSelected in ANode.States) then
                    Selections.Add(ANode);
                end;
              end;
            end;
...
var
  ...
  begin
    ...
    if R.Right > AClientRect.Right then
      R.Right := AClientRect.Right;
    if not R.EqualsTo(FInplaceEditor.GetBounds) then
      FInplaceEditor.SetBounds(R);
    if not FInplaceEditor.Visible then
      FInplaceEditor.Show;
  end;
...
begin
  ...
  begin
    ...
    begin
      if FocusChanging(nil, FocusColumn) then
      begin
        try
          if Assigned(FFocusNode) then
            FFocusNode.FStates := FFocusNode.FStates -
              [TQVTNodeState.nsFocused];
          FFocusNode := nil;
        finally
          FocusChanged;
        end;
      ...
      // else
      // DebugOut('Focus not allow at SetFocusNode');
    end;
  end;
end;
...
function TQVTColumn.GetIsEditing: Boolean;
begin
  Result := Assigned(TreeView.FInplaceEditor) and
    (TreeView.FInplaceEditor.Column = Index);
end;
...
var
  ANext: TQVTNode;
begin
  if TreeView.CanDeleteNode(Self) then
  begin
    ANext := Next;
    FParent.Dirty(ANext);
...
function TQVTNode.GetIsEditing: Boolean;
begin
  with TreeView do
    Result := Assigned(FInplaceEditor) and (FInplaceEditor.Node = Self);
end;
...
function TQVTNode.GetTreeView: TQVirtualTreeView;
begin
  assert(Assigned(FTreeView), 'Treeview not assigned');
  Result := FTreeView;
end;

function TQVTNode.GetVisible: Boolean;
begin
  Result := TQVTNodeState.nsVisible in FStates;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if AChild = TreeView.FFocusingNode then
          begin
            TreeView.FFocusingNode := nil;
            TreeView.FStates := TreeView.FStates - [tsFocusChanging];
          end;
...
begin
  ...
  if CanFocus then
  begin
    if ATreeView.FocusChanging(Self, TreeView.FocusColumn) then
    begin
      // Expand all parent
      ...
      try
        ...
        if Assigned(ATreeView.FFocusNode) then
          ATreeView.FFocusNode.States := ATreeView.FFocusNode.States -
            [nsFocused];
        ATreeView.FFocusNode := Self;
      finally
        ATreeView.FocusChanged;
        if AContentChanged then
          ATreeView.NodeContentChanged;
      end;
    end;
    // else
    // DebugOut('Focus changed failed on TQVTNode.SetFocus');
    ScrollToView;
  end
  else
    ATreeView.FStates := ATreeView.FStates - [tsFocusChanging];
end;
...
procedure TQVTNode.SetVisible(const value: Boolean);
begin
  if value then
    SetStates(FStates + [TQVTNodeState.nsVisible])
  else
    SetStates(FStates - [TQVTNodeState.nsVisible]);
end;
...
begin
  ...
  begin
    ...
    if Assigned(TQVTCustomTextCell(src).FTextSettings) then
      TextSettings.Assign(TQVTCustomTextCell(src).TextSettings);
  end;
end;
...
var
  ...
  AIndent: Single;
  ACheckBounds: TQCheckBounds;
begin
  if Supports(Node.CellDrawer[Column], IQVTStateDrawer, ADrawer) and
    Node.ExtByType(TQCheckBounds, ACheckBounds) and
    ACheckBounds.Bounds.Contains(APos) then
  begin
    AStates := CheckStates;
    if TQVTCheckState.csChecked in AStates then
...
var
  ARect: TQCheckBounds;
begin
  ARect := Node.ExtByType(TQCheckBounds) as TQCheckBounds;
  if not Assigned(ARect) then
  begin
    ARect := TQCheckBounds.Create;
    Node.Exts.Add(ARect);
  end;
  ARect.Bounds := R;
  FCheckBounds := R;
...
begin
  if (FRefCount and $80000000) = 0 then
  begin
    inherited;
    FRefCount := FRefCount or $80000000;
    TMessageManager.DefaultManager.SendMessage(Self,
...
begin
  // DebugOut('Editor %x free', [IntPtr(Self)]);
  inherited;
end;
...
function TQVTBaseEditor.GetBounds: TRectF;
begin
  Result := FBounds;
end;

function TQVTBaseEditor.GetColumn: Integer;
begin
  Result := FColumn;
end;

function TQVTBaseEditor.GetColumnId: Integer;
begin
  if (FColumn >= 0) and (FColumn < FNode.TreeView.Header.Columns.Count) then
    Result := FNode.TreeView.Columns[FColumn].ID
  else
    Result := -1;
end;
...
function TQVTBaseEditor.GetIsFocused: Boolean;
begin
  Result := Assigned(FControl) and FControl.IsFocused;
end;

function TQVTBaseEditor.GetNode: TQVTNode;
begin
  Result := FNode;
end;

function TQVTBaseEditor.GetVisible: Boolean;
begin
...
procedure TQVTBaseEditor.SetVisible(const AValue: Boolean);
begin
...
begin
  ...
  begin
    if (FRefCount and $40000000) = 0 then
      Inc(FRefCount);
    Result := FRefCount;
  end
  else
...
begin
  if (not Assigned(Owner)) and ((FRefCount and $C0000000) = 0) then
  begin
    Dec(FRefCount);
    Result := FRefCount;
    if FRefCount = 0 then
    begin
      FRefCount := FRefCount or $40000000;
      DisposeOf;
    end;
  end
  else
...
function TQVTTextEditor.GetText: String;
begin
...
procedure TQVTTextEditor.SetText(const AValue: String);
begin
  if Assigned(Control) then
    (Control as TEdit).Text := AValue;
end;
    ...
    begin
      ...
      try
        ...
        begin
          if TextEditable then
          begin
            AIdx := AList.IndexOf(AText);
            if AIdx = -1 then
              TComboEdit(Control).Text := AText
            else
              TComboEdit(Control).ItemIndex := AIdx;
          end
          else
            TComboBox(Control).ItemIndex := AList.IndexOf(AText);
        end
        else
        begin
          if TextEditable then
...
function TQVTListEditor.GetItemIndex: Integer;
begin
  if Assigned(Control) then
  begin
    if TextEditable then
      Result := TComboEdit(Control).ItemIndex
    else
...
function TQVTListEditor.GetItemObject: TObject;
var
  AItemIndex: Integer;
begin
  Result := nil;
  if Assigned(Control) then
  begin
    AItemIndex := GetItemIndex;
    if AItemIndex <> -1 then
      Result := PickList.Objects[GetItemIndex]
  end;
end;

function TQVTListEditor.GetPickList: TStrings;
begin
  if Assigned(Control) then
  begin
    if TextEditable then
      Result := TComboEdit(Control).Items
    else
...
function TQVTListEditor.GetText: String;
begin
  if Assigned(Control) then
  begin
    if TextEditable then
      Result := TComboEdit(Control).Text
    else
    begin
...
procedure TQVTListEditor.SetItemIndex(const AValue: Integer);
begin
  if Assigned(Control) then
  begin
    if TextEditable then
      TComboEdit(Control).ItemIndex := AValue
    else
      TComboBox(Control).ItemIndex := AValue;
  end;
end;

procedure TQVTListEditor.SetPickList(const value: TStrings);
var
  AList: TStrings;
begin
  AList := PickList;
...
procedure TQVTListEditor.SetText(const AValue: String);
begin
  if Assigned(Control) then
  begin
    if TextEditable then
      TComboEdit(Control).Text := AValue
    else
      with TComboBox(Control) do
