...
type
  ...
  public
    constructor Create(const AValues: UnicodeString; ADelimiter: Char = #0); overload;
  ...
  TSerializeNameFormat =
      (Normal, LowerCamel, UpperCamel, Underline, UpperCase, LowerCase, UpperCaseUnderLine, LowerCaseUnderLine);
  ...
  public
    ...
    property NameFormat: TSerializeNameFormat read FNameFormat write FNameFormat;
  end;
  ...
  TSerializeDateTimeFormat = (AutoDetect, DateTimeString, UnixTimeStamp, UnixTimeStampMs, DateString, TimeString);
  ...
  public
    constructor Create(const AFormat: TSerializeDateTimeFormat; const AFormatText: UnicodeString = '');
  ...
  ValueRangeAttribute < TQValueType
      >= class(TQValidatorAttribute) public constructor Create(
          const AMinValue, AMaxValue: TQValueType;
          const AErrorMsg: UnicodeString = '';
          ABounds: TQRangeBounds = []); end;
  // 长度限制，如 [LengthRange<UnicodeString>(4,12,'[Size] out of range [MinSize] to [MaxSize]')]
  LengthRangeAttribute < TQValueType
      >= class(TQValidatorAttribute) public
      constructor Create(const AMinSize, AMaxSize: SizeInt; const AErrorMsg: UnicodeString = ''); end;
  // 默认值，指明如果序列化时，没有赋值时如何处理，如果不指定，默认为 Default<TQValueType>，用例:[DefaultValue<String>('male')]
  DefaultValueAttribute < TQValueType >= class(TCustomAttribute) private FValue: TQValueType; public
      constructor Create(const AValue: TQValueType); overload; property Value:
      TQValueType read FValue; end;
      // 序列化时忽略该项，用例：[Ignore]
      IgnoreAttribute = class(TCustomAttribute) end;
      // 序列化时强制写入，用例：[NoDefault]
      NoDefaultAttribute = class(TCustomAttribute) end;
      // 格式校验规则，如 [Validator('email')]指明值使用 email 验证器进行验证
      ValidatorAttribute = class(TQValidatorAttribute) private FErrorMsg:
      string; public constructor Create(const AName: UnicodeString; const AErrorMsg: UnicodeString = ''); overload; end;
      // 前缀，如 clWhite 指定前缀为 cl，则实际保存时，值被保存为 White，前缀和后缀可能有多个，如:
      // clSnow,clWebSnow，前缀分别是 cl,wlWebSnow，程序序列化时，优先使用较长前缀
      PrefixAttribute = class(TQTextAttribute) public property Prefix:
      UnicodeString read FText; end;
      // 尾缀，如 DynamicOrder 指定后缀为 Order，则实际保存时，值被序列化为 Dynamic
      TailAttribute = class(TQTextAttribute) public property Tail:
      UnicodeString read FText; end;
      ...
      TQValueStorage; public constructor Create(const AStorage: TQValueStorage); overload; property Storage:
      TQValueStorage read FStorage; end;
      // 正则表达式规则验证，如 [RegexValidator('^\d+$')]
      RegexValidatorAttribute = class(TQValidatorAttribute) public
      constructor Create(const ARegexpr: UnicodeString; const AErrorMsg: UnicodeString = ''); overload; end;
      ...
      TransformAttribute = class(TQTextAttribute) public property Name:
      UnicodeString read FText; end;
      ...
      TQMultiThreadMode = (MainThread, SingleThread, MultiThread); ThreadModeAttribute = class(TCustomAttribute)
      private FMode:
      TQMultiThreadMode; public constructor Create(AMode: TQMultiThreadMode); overload; property Mode:
      TQMultiThreadMode read FMode; end; implementation
      { ValueRangeAttribute<TQValueType> } constructor ValueRangeAttribute<TQValueType>
          .Create(const AMinValue, AMaxValue: TQValueType; const AErrorMsg: UnicodeString; ABounds: TQRangeBounds);
      begin inherited Create; FValidator :=
      TQRangeValidator<TQValueType>
          .Create(AMinValue, AMaxValue, AErrorMsg, nil); TQRangeValidator<TQValueType>(FValidator).Bounds :=
      ABounds; end;
      { LengthRangeAttribute<TQValueType> } constructor LengthRangeAttribute<TQValueType>
          .Create(const AMinSize, AMaxSize: SizeInt; const AErrorMsg: UnicodeString); begin inherited
      Create; FValidator :=
      TQLengthValidator<TQValueType>.Create(AMinSize, AMaxSize, AErrorMsg); end;
      { DefaultValueAttribute<TQValueType> } constructor DefaultValueAttribute<TQValueType>
          .Create(const AValue: TQValueType); begin inherited Create; FValue :=
      AValue; end;
      { ValidatorAttribute } constructor ValidatorAttribute.Create(const AName, AErrorMsg: UnicodeString);
      begin inherited Create; FValidator :=
      TQValidators.Custom<UnicodeString>(AName) as TQTextValidator; FErrorMsg :=
      AErrorMsg; end;
      { RegexValidatorAttribute } constructor RegexValidatorAttribute
          .Create(const ARegexpr, AErrorMsg: UnicodeString); begin inherited Create; FValidator :=
      TQRegexValidator.Create(AErrorMsg); (FValidator.AsValidator as TQRegexValidator).Regex :=
      ARegexpr; end;
      { ThreadModeAttribute } constructor ThreadModeAttribute.Create(AMode: TQMultiThreadMode);
      begin inherited Create; FMode :=
      AMode; end;
      { TQTextAttribute }
      constructor TQTextAttribute.Create(const AText: UnicodeString); begin inherited Create; FText :=
      AText; end;
      { ValueListAttribute<TQValueType> }
      { ValueStorageAttribute } constructor ValueStorageAttribute.Create(const AStorage: TQValueStorage);
      begin inherited Create; FStorage :=
      AStorage; end;
      { NameFormatAttribute }
      constructor NameFormatAttribute.Create(AFormat: TSerializeNameFormat); begin inherited Create; FNameFormat :=
      AFormat; end;
      { DateTimeFormatAttribute } constructor DateTimeFormatAttribute
          .Create(const AFormat: TSerializeDateTimeFormat; const AFormatText: string);
      begin inherited Create; FFormat :=
      AFormat; FFormatText :=
      AFormatText; end;
      { StringListAttribute } constructor StringListAttribute
          .Create(const AValues: UnicodeString; ADelimiter: Char); begin if ADelimiter = #0 then FValues :=
      AValues.Split([',', ';', ' ', #9], TStringSplitOptions.ExcludeEmpty)
      else FValues :=
      AValues.Split([ADelimiter], TStringSplitOptions.ExcludeEmpty) end;
      { StringPairAttribute } constructor StringPairAttribute.Create(const AKey, AValue: UnicodeString); begin inherited
      Create; FKey := AKey; FValue := AValue; end; end.
