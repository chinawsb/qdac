...
{$IFEND >=2010}
  ...
  TJsonEncodeSetting = (jesIgnoreNull, jesIgnoreDefault, jesDoFormat,
    ...
    jesEscapeSlashes, jes1Element1Line);
...
{$ENDIF}
  ...
  protected
    ...
    procedure InternalSetAsBytes(AConverter: TQJsonEncodeBytesEvent;
      ABytes: TBytes);
    function InternalFlatItems(var AItems: TQStringArray; AFlatPart: Integer;
      AOptions: TQCatOptions): Integer;
  public
...
{$IFEND >=2010}
    ...
    function EncodeToBytes(AEncoding: TTextEncoding;
      ASettings: TJsonEncodeSettings; AIndent: QStringW = '  '): TBytes;
...
{$IFEND}
    ...
    function CatValues(const AQuoter, ADelimiter: QCharW;
      AOptions: TQCatOptions): QStringW;
    ...
    function ValuesToStrings(AList: TStrings; AOptions: TQCatOptions)
      : Integer; overload;
    function ValuesToStrings(AOptions: TQCatOptions): TQStringArray; overload;
    procedure ValuesFromStrings(const AValues: TQStringArray;
      AIsReplace: Boolean = True; AStartIndex: Integer = 0;
      ACount: Integer = MaxInt); overload;
    procedure ValuesFromStrings(AList: TStrings; AIsReplace: Boolean = True;
      AStartIndex: Integer = 0; ACount: Integer = MaxInt); overload;
    procedure ValuesFromIntegers(const AValues: TIntArray;
      AIsReplace: Boolean = True; AStartIndex: Integer = 0;
      ACount: Integer = MaxInt);
    procedure ValuesFromInt64s(const AValues: TInt64Array;
      AIsReplace: Boolean = True; AStartIndex: Integer = 0;
      ACount: Integer = MaxInt);
    procedure ValuesFromFloats(const AValues: TFloatArray;
      AIsReplace: Boolean = True; AStartIndex: Integer = 0;
      ACount: Integer = MaxInt);
...
function TQJson.CatNames(const AQuoter, ADelimiter: QCharW;
  AOptions: TQCatOptions): QStringW;
var
...
function TQJson.CatValues(const AQuoter, ADelimiter: QCharW;
  AOptions: TQCatOptions): QStringW;
var
  AItems: TQStringArray;
  ACount: Integer;
...
function TQJson.EncodeToBytes(AEncoding: TTextEncoding;
  ASettings: TJsonEncodeSettings; AIndent: QStringW): TBytes;
var
  AText: String;
...
// 修正XE6中System.rtti中TValue对tkSet类型处理的Bug
  ...
  begin
    ...
    begin
      ...
      begin
        // 接口、函数和无类型指针无法处理
        if AFields[J].FieldType.Handle^.Kind in [tkUnknown, tkInterface,
          tkMethod, tkProcedure, tkClassRef, tkPointer] then
          continue;
        Add(AFields[J].Name)
          .FromRtti(PByte(IntPtr(ASource) + AFields[J].Offset),
...
{$IFEND}
            case APropList[J].PropType^.Kind of
              ...
              tkRecord, tkArray, tkDynArray{$IFDEF MANAGED_RECORD},
                tkMRecord{$ENDIF}: // 记录、数组、动态数组属性系统也不保存，也没提供所有太好的接口
...
function TQJson.InternalFlatItems(var AItems: TQStringArray; AFlatPart: Integer;
  AOptions: TQCatOptions): Integer;
var
      ...
      begin
        ...
        begin
          if (AItems[ADupIndex] = AValue) or
            ((coIgnoreCase in AOptions) and (CompareText(AItems[ADupIndex],
            AValue) = 0)) then
          begin
            AFound := True;
            Break;
...
begin
  ...
  else if AInstance.Kind in [tkRecord{$IFDEF MANAGED_RECORD}, tkMRecord{$ENDIF}]
  then
...
function TQJson.ValuesToStrings(AList: TStrings;
  AOptions: TQCatOptions): Integer;
var
  AItems: TQStringArray;
  AIndex: Integer;
...
procedure TQJson.ValuesFromStrings(const AValues: TQStringArray;
...
procedure TQJson.ValuesFromIntegers(const AValues: TIntArray;
  AIsReplace: Boolean; AStartIndex, ACount: Integer);
begin
...
procedure TQJson.ValuesFromStrings(AList: TStrings; AIsReplace: Boolean;
  AStartIndex, ACount: Integer);
begin
  DataType := jdtArray; // 必需是数组
  if AIsReplace then
