...
type
  ...
  TQInterpolationType = (Linear, Quadratic, Cubic, Quartic, Quintic, Sinusoidal,
    Exponential, Circular, Elastic, Back, Bounce);
  TQAnimationEvent = (Starting, Started, Stopping, Stopped, NextLoop, Disabled,
    Enabled, Paused, Resumed, GotoTime, EnterDelay, ExitDelay);
  ...
  IQAnimation = interface
    ...
    property Interpolation: TQInterpolationType read GetInterpolation
  ...
  TQAnimationNotifyEvent = procedure(Sender: IQAnimation;
    AEvent: TQAnimationEvent) of object;
  TQAnimationNotifyCallback = reference to procedure(Sender: IQAnimation;
    AEvent: TQAnimationEvent);

  TQAniHost = class(TComponent)
  private
    FAnimation: IQPropertyAnimation;
  public
    class function BindAnimation(AOwner: TComponent;
      Animation: IQPropertyAnimation): TQAniHost;
    class function AnimationHost(AOwner: TComponent; ACreateIfMissed: Boolean)
      : TQAniHost;
...
function CreateIntAnimation(ACallback: TQAnimationNotifyEvent)
  : IQIntAnimation; overload;
// 布尔动画处理
function CreateBooleanAnimation(ACallback: TQAnimationNotifyCallback)
  : IQBoolAnimation; overload;
function CreateBooleanAnimation(ACallback: TQAnimationNotifyEvent)
  : IQBoolAnimation; overload;
// 颜色动画处理
...
type
  ...
  public
    ...
    destructor Destroy; override;
    property OnAnimationEvent: TQAnimationNotifyEvent read FOnAnimationEvent
      write FOnAnimationEvent;
    property AnimationType: TQAnimationType read FType write FType;
    property Duration: Single read FDuration write SetDuration;
    property Interpolation: TQInterpolationType read FInterpolation
  ...
  TAlphaColorAnimation = class(TRangeAnimation<TAlphaColor>,
  ...
  protected
    ...
    function GetPropertyInstance: Pointer;
  public
    constructor Create(AOwner: TObject; APath: String;
      ACallback: TQAnimationNotifyCallback); overload;
    constructor Create(AOwner: TObject; APath: String;
      ACallback: TQAnimationNotifyEvent); overload;
  end;
  ...
  TFloatPropertyAnimation = class(TRangePropertyAnimation<Double>,
    IQFloatAnimation)
  protected
    function GetCurrentVal: Double; override;
  ...
  TColorPropertyAnimation = class(TRangePropertyAnimation<TColor>,
    IQColorAnimation)
  protected
    function GetCurrentVal: TColor; override;
  ...
  TAlphaColorPropertyAnimation = class(TRangePropertyAnimation<TAlphaColor>,
    IQAlphaColorAnimation)
  protected
    function GetCurrentVal: TAlphaColor; override;
...
function ConvertCallback(ACallback: TQAnimationNotifyCallback)
  : TQAnimationNotifyEvent;
var
...
function CreatePropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyCallback): IQPropertyAnimation;
begin
  Result := TPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreatePropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyEvent): IQPropertyAnimation;
begin
  Result := TPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateIntPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyCallback): IQIntAnimation;
begin
  Result := TIntPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateIntPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyEvent): IQIntAnimation;
begin
  Result := TIntPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateFloatPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyCallback): IQFloatAnimation;
begin
  Result := TFloatPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateFloatPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyEvent): IQFloatAnimation;
begin
  Result := TFloatPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateColorPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyCallback): IQColorAnimation;
begin
  Result := TColorPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateColorPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyEvent): IQColorAnimation;
begin
  Result := TColorPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateAlphaColorPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyCallback): IQAlphaColorAnimation;
begin
  Result := TAlphaColorPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function CreateAlphaColorPropertyAnimation(AObject: TObject; APropPath: String;
  ACallback: TQAnimationNotifyEvent): IQAlphaColorAnimation;
begin
  Result := TAlphaColorPropertyAnimation.Create(AObject, APropPath, ACallback);
end;

function GetNormalizeColor(AStart, AStop: TColor; ATime: Single)
  : TColor; overload;
var
...
begin
  AResultColor.R :=
    Trunc(AStartColor.R + (AStopColor.R - AStartColor.R) * ATime);
  AResultColor.G :=
    Trunc(AStartColor.G + (AStopColor.G - AStartColor.G) * ATime);
  AResultColor.B :=
    Trunc(AStartColor.B + (AStopColor.B - AStartColor.B) * ATime);
  AResultColor.A :=
    Trunc(AStartColor.A + (AStopColor.A - AStartColor.A) * ATime);
end;

function GetNormalizeColor(AStart, AStop: TAlphaColor; ATime: Single)
  : TAlphaColor; overload;
var
...
begin
  if (FPauseCount = 0) and (FDisableCount = 0) and
    ((FFlags and AF_STARTED) <> 0) and Assigned(TAnimationTimer.Current) then
...
constructor TPropertyAnimation.Create(AOwner: TObject; APath: String;
  ACallback: TQAnimationNotifyEvent);
begin
    ...
    begin
      ...
      begin
        ...
        case Result.PropertyType.TypeKind of
          ...
          tkInteger, tkChar, tkEnumeration, tkFloat, tkString, tkSet, tkMethod,
            tkWChar, tkLString, tkWString, tkInt64, tkUString:
            begin
...
class function TQAniHost.AnimationHost(AOwner: TComponent;
  ACreateIfMissed: Boolean): TQAniHost;
var
  I: Integer;
begin
...
class function TQAniHost.BindAnimation(AOwner: TComponent;
  Animation: IQPropertyAnimation): TQAniHost;
var
  I: Integer;
begin
